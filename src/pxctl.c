...
#define REGSALL(_cmd)                                                          \
  _cmd(r15) _cmd(r14) _cmd(r13) _cmd(r12) _cmd(rbp) _cmd(rbx) _cmd(r11)        \
      _cmd(r10) _cmd(r9) _cmd(r8) _cmd(rax) _cmd(rcx) _cmd(rdx) _cmd(rsi)      \
          _cmd(rdi) _cmd(orig_rax) /*_cmd(rip)*/                               \
      _cmd(cs) _cmd(eflags) _cmd(rsp) _cmd(ss) _cmd(fs_base) _cmd(gs_base)     \
          _cmd(ds) _cmd(es) _cmd(fs) _cmd(gs)
...
static inline int zprtok(int lvl) { return (_USE_ZPRT_ && (opt_d >= lvl)); }
...
long ptxcmd(enum __ptrace_request cmd, void *addr, void *data) {
  long ret;
...
void ptxwait(const char *reason) {
